        ...
        <tbody class="bg-white dark:divide-gray-700 dark:bg-gray-800">
          <template v-for="(item, index) in copyData" :key="item">
            ...
            >
                ...
                <template v-else>
                  <td v-if="object.value != ''" :class="`px-4 py-4 text-sm `">
                    ...
                    <span v-else-if="object.money">
                     ...
                     <div :style="`height: 25px; width: 25px; background-color: ${item[object.value]}; border-radius: 5px`"></div>
                    </span>
                    <span
                      v-else
                      :class="` ${
                      ...
                      >{{
                        object.deep
                          ? deep(item, object.value).length > 0
                            ...
                            : ""
                          : object.badge
                          ? item[object.value] === 1 ? "Ativado" : item[object.value]
                          : item[object.value]
                      }}</span
                    >
                  </td>
              ...
              <td class="flex relative h-full items-center">
                <div
                  class="flex h-full absolute top-0 justify-start items-center space-x-4 text-sm"
                >
                  <template v-for="option in options" :key="option.icon">
                    <div @click="option.action(item)">
                      <i
                        :class="`gg-${option.icon} mx-3 cursor-pointer hover:text-primary`"
                      ></i>
                    </div>
                  </template>
                </div>
...
export default {
  ...
  setup(props: any) {
    ...
    const exportCSVFile = (headers, items, fileTitle) => {
      ...
      const data = [...items]
      const head = headers.filter(el => el.key != "Options")
